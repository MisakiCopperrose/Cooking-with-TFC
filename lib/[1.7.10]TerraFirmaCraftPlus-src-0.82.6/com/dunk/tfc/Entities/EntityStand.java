...
public class EntityStand extends EntityLiving
{
	...
	protected void entityInit()
	{
		...
		this.dataWatcher.addObject(start + EntityStand.defaultEquipableLength + EntityStand.defaultArmorLength,
				new Float(1));
		this.dataWatcher.addObject(start + EntityStand.defaultEquipableLength + EntityStand.defaultArmorLength + 1,
				Integer.valueOf(0));
	}
	...
	public void onUpdate()
	{
		...
		// this.getLookHelper().setLookPosition(this.posX + lookX, this.posY +
		// (double)this.getEyeHeight(), this.posZ + lookZ, 10.0F,
		// (float)this.getVerticalFaceSpeed());
		// this.getNavigator().tryMoveToXYZ(this.posX + lookX, this.posY,
		// this.posZ + lookZ, 0.5);
	}
	...
	/**
	 * Imported from EntityLivingBase because this does not exist on the server
	 * apparently
	 */
	...
	public boolean interact(EntityPlayer ep)
	{
		if (!worldObj.isRemote)
		{
			...
			if (is != null)
			{
				...
				else if (is.getItem() instanceof IEquipable && !(is.getItem() instanceof ItemBlock))
				{
					IEquipable tempItem = (IEquipable) is.getItem();
					EquipType et = tempItem.getEquipType(is);
					int slot = 0;
					boolean armor = false;
					switch (et)
					{
					...
					if (slot > -1)
					{
						...
						{
							if (this.equipable[slot] == null)
							{
								this.setEquipable(slot, is);
								is.stackSize--;
							}
							ep.setCurrentItemOrArmor(0, is);
						}
			...
			else
			{
				...
				double angleTan = hitVec.yCoord / Math
						.sqrt(hitVec.xCoord * hitVec.xCoord + hitVec.zCoord * hitVec.zCoord);
				...
				if (slot != -1)
				{
					...
					{
						if(slot == 2 && equipable[4-slot]==null)
						{
							slot = 4;
						}
						ItemStack i = equipable[4-slot];
						equipable[4-slot] = null;
						giveItemToPlayer(worldObj, ep, i);
					}
