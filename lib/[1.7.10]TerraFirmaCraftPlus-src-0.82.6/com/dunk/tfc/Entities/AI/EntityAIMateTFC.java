...
public class EntityAIMateTFC extends EntityAIBase
{
	...
	{
		if(this.theAnimal instanceof IAnimal)
		{
			((IAnimal)theAnimal).setMating(false);
		}
		targetMate = null;
		matingCounter = 0;
	}
	...
	public void updateTask ()
	{
		...
		if(this.theAnimal instanceof IAnimal )
		{
			if(((IAnimal)theAnimal).getSleepTimer() > 50)
			{
				matingCounter++;
			}
			else if(!((IAnimal)theAnimal).isSleeping() && theAnimal.getEntity().getDistanceSqToEntity(this.targetMate.getEntity()) < 9.0D)
			{
				((IAnimal)theAnimal).goToSleep();
				((IAnimal)theAnimal).setMating(true);
			}
		}
		else
		{
