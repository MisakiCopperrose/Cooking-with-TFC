...
public class EntityHorseTFC extends EntityHorse implements IInvBasic, IAnimal, ICausesDamage
{
	...
	private static final IAttribute HORSE_JUMP_STRENGTH = new RangedAttribute("horse.jumpStrengthTFC", 0.7D, 0.0D, 2.0D)
			.setDescription("Jump StrengthTFC").setShouldWatch(true);
	...
	public EntityHorseTFC(World par1World)
	{
		...
		this.tasks.addTask(1, new EntityAINearestRetaliateTarget(this, EntityLivingBase.class, 1,1.5, true));
		...
		this.tasks.addTask(1, new EntityAIRunAroundLikeCrazy(this, 1.8D));
	...
	{
		float f = TFC_MobData.HORSE_DAMAGE;
		int i = 3;
	...
	protected void applyEntityAttributes()
	{
		...
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(TFC_MobData.HORSE_HEALTH * sizeMod * strengthMod);// MaxHealth
	...
	{
		return 0.4000000059604645D + this.rand.nextDouble() * 0.2D + this.rand.nextDouble() * 0.2D + this.rand
				.nextDouble() * 0.2D;
	}
	...
	{
		return (0.44999998807907104D + this.rand.nextDouble() * 0.3D + this.rand.nextDouble() * 0.3D + this.rand
				.nextDouble() * 0.3D) * 0.25D;
	}
	...
	public String getCommandSenderName()
	{
		...
		if (isDomesticated)
		{
			...
			{
			case 0:
				return TFC_Core.translate("entity.horseTFC.name");
			case 1:
				return TFC_Core.translate("entity.donkeyTFC.name");
			case 2:
				return TFC_Core.translate("entity.muleTFC.name");
			}
		...
		{
		case 0:
			return TFC_Core.translate("entity.wildHorseTFC.name");
		case 1:
			return TFC_Core.translate("entity.wildAssTFC.name");
		case 3:
			return TFC_Core.translate("entity.zebraTFC.name");
		}
		return TFC_Core.translate("entity.wildHorseTFC.name");
	}
	...
	public boolean canMateWith(EntityAnimal animal)
	{
		...
		else
		{
			...
			if (this.getBreedable() && entityhorse
					.getBreedable() && ((IAnimal) animal).isDomesticated() == this.isDomesticated)
			{
				int i = this.getHorseType();
				int j = entityhorse.getHorseType();
				return (i == j && i != 2) || (i == 0 && j == 1 && ((IAnimal) animal)
						.isDomesticated() && isDomesticated()) || (i == 1 && j == 0 && ((IAnimal) animal)
	...
	public boolean canMateWith(IAnimal animal)
	{
		return animal.getGender() != this.getGender() && this.isAdult() && animal
				.isAdult() && animal instanceof EntityHorseTFC;
	}
	...
	public void clearLeashed(boolean par1, boolean par2)
	{
		...
		{
			// ItemStack item =
			// ((EntityPlayer)entity).inventory.getCurrentItem();
	...
	public EntityAgeable createChildTFC(EntityAgeable eAgeable)
	{
		...
		double healthSum = this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
				.getBaseValue() + this.mateMaxHealth + this.calcMaxHealth();
		...
		double jumpSum = this.getEntityAttribute(HORSE_JUMP_STRENGTH).getBaseValue() + this.mateJumpStrength + this
				.calcJumpStrength();
		...
		double speedSum = this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
				.getBaseValue() + this.mateMoveSpeed + this.calcMoveSpeed();
	...
	protected void entityInit()
	{
		...
		this.dataWatcher.addObject(26, Integer.valueOf(0)); // Size, strength,
															// aggression,
															// obedience
		this.dataWatcher.addObject(24, Integer.valueOf(0)); // familiarity,
															// familiarizedToday,
															// pregnant, empty
															// slot
		this.dataWatcher.addObject(25, String.valueOf("0")); // Time of
																// conception,
																// stored as a
																// string since
																// we can't do
	...
	public void familiarize(EntityPlayer ep)
	{
		...
		if ((this.riddenByEntity == null || !this.riddenByEntity
				.equals(ep)) && !familiarizedToday && stack != null && this.isFood(stack) && canFamiliarize())
		{
	...
	{
		return this.riddenByEntity == null && this.ridingEntity == null && this.isTame() && this.isAdultHorse() && !this
				.func_110222_cv()/* Not a Mule, Zombie or Skeleton */ && this.getHealth() >= this.getMaxHealth();
	}
	...
	protected EntityHorseTFC getClosestHorse(Entity entity, double range)
	{
		...
		List<EntityHorseTFC> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(entity,
				entity.boundingBox.addCoord(range, range, range), HORSE_BREEDING_SELECTOR);
	...
	// We use this to catch the EntityLiving check, so that other interactions
	// can be performed on leashed animals
	@Override
	public boolean getLeashed()
	{
		if (super.getLeashed() && getLeashedToEntity() instanceof EntityPlayer && ((EntityPlayer) getLeashedToEntity()).inventory
				.getCurrentItem() == null && func_110174_bM()/* maximumHomeDistance */ != -1)
		{
			return false;
		}
	...
	{
		int i = this.getHorseType();
		return this.isChested() && (i == 1 || i == 2) ? 17 : 2; // 2 (Armor &
																// Saddle) + 15
																// Donkey Chest
																// Slots
	}
	...
	{
		temper *= 5; // This is because we want obedience_mod and aggression_mod
						// to have an effect
		int j = MathHelper.clamp_int(this.getTemper() + (int) (temper * obedienceMod * (1f / aggressionMod)), 0,
				this.getMaxTemper());
		this.setTemper(j);
		return j;
	}
	...
	public boolean interact(EntityPlayer player)
	{
		...
		if (itemstack != null && this.isBreedingItemTFC(itemstack) && checkFamiliarity(InteractionEnum.BREED,
				player) && this
						.getGrowingAge() == 0 && !super.isInLove() && (this.familiarizedToday || !canFamiliarize()))
		{
		...
		else
		{
			if (itemstack != null)
			{
				...
				if (this.func_110229_cs()/* Donkey or Mule */ && !this.isChested() && itemstack.getItem() == Item
						.getItemFromBlock(TFCBlocks.chest))
				{
	...
	public void onLivingUpdate()
	{
		...
		if (hunger > 0)
			hunger--;
		if (this.getHorseType() == 1)
		{
			// System.out.println("Ass!" + this.posX + ", " + this.posZ);
		}
		if (this.riddenByEntity != null && this.riddenByEntity instanceof EntityPlayer && rand
				.nextInt(600) == 0 && !familiarizedToday && canFamiliarize())
		{
	...
	{
		Entity temp = player.ridingEntity; // Trick the player into thinking
											// they are riding the horse so we
											// can track that specific horse

		if (temp == null) // Shift-right click while not riding to open
							// inventory
		{
	...
	private void openHorseContainer(EntityPlayer player)
	{
		...
		{
			player.openGui(TerraFirmaCraft.instance, 42, player.worldObj, (int) player.posX, (int) player.posY,
					(int) player.posZ);
		}
	}
