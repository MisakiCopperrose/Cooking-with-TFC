...
public class EntitySheepTFC extends EntitySheep implements IShearable, IAnimal, ICausesDamage
{
	/**
	...
	public EntitySheepTFC(World par1World)
	{
		...
		this.tasks.addTask(1, new EntityAINearestRetaliateTarget(this, EntityLivingBase.class, 1,1.6, true));
		...
		this.tasks.addTask(7, new EntityAIWatchClosest(this, EntitySheepTFC.class, 6.0F));
	...
	protected void applyEntityAttributes()
	{
		...
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(TFC_MobData.MOUFLON_HEALTH);// MaxHealth
	}
	...
	{
		float f = TFC_MobData.MOUFLON_DAMAGE;
		int i = 4;
	...
	public boolean canMateWith(IAnimal animal)
	{
		return animal.getGender() != this.getGender() && this.isAdult() && animal
				.isAdult() && animal instanceof EntitySheepTFC && ((IAnimal) animal)
	...
	public int combineColors(EntityAnimal parent, int mateColor)
	{
		...
		ItemStack itemstack = CraftingManager.getInstance().findMatchingRecipe(this.colorCrafting,
				((EntitySheep) parent).worldObj);
		int babyColor;
	...
	protected void dropFewItems(boolean par1, int par2)
	{
		...
		if (!this.getSheared() && isDomesticated())
		{
			this.entityDropItem(
		...
		{
			this.entityDropItem(new ItemStack(TFCItems.fur, 1, Math.max(0, Math.min(2, (int) (ageMod * sizeMod*1.1f)))),
					0.0F);
		}
		else
		{
			this.entityDropItem(new ItemStack(TFCItems.hide, 1, Math.max(0, Math.min(2, (int) (ageMod * sizeMod)))),
					0.0F);
		}
	...
	{
		if (isDomesticated || TFC_Climate.getHeightAdjustedTemp(worldObj, (int) posX, (int) posY, (int) posZ) < 10)
		{
	...
	public String getCommandSenderName()
	{
		if (isDomesticated)
		{
			return TFC_Core.translate("entity.sheepTFC.name");
		}
		return TFC_Core.translate("entity.mouflonTFC.name");
	}
	...
	public boolean interact(EntityPlayer player)
	{
		if (!worldObj.isRemote)
		{
			...
			boolean familiarEnough = false;
			if (player.getHeldItem() != null && player.getHeldItem().getItem() instanceof ItemKnife)
			{
				familiarEnough = this.checkFamiliarity(InteractionEnum.SHEAR, player);
			}
			// Shearing with a pair of shears is handled with ItemShears
			if (player.getHeldItem() != null && player.getHeldItem()
					.getItem() instanceof ItemKnife && !getSheared() && familiarEnough && isAdult())
			{
				...
				setSheared(true);
				this.entityDropItem(new ItemStack(TFCItems.wool, isDomesticated() ? 2 : 1), 0.0F);
				if (!player.capabilities.isCreativeMode)
					player.getHeldItem().damageItem(1, player);
			}
			else if (player.getHeldItem() != null && player.getHeldItem()
					.getItem() instanceof ItemKnife && !familiarEnough && isAdult())
			{
				if (this.rand.nextInt(10) == 0)
				{
					this.setRevengeTarget(player);
					TFC_Core.sendInfoMessage(player, new ChatComponentTranslation("entity.notFamiliarAngry"));
				}
			}
		}
	...
	public ArrayList<ItemStack> onSheared(ItemStack item, IBlockAccess world, int x, int y, int z, int fortune)
	{
		...
		setSheared(true);
		int num = isDomesticated() ? 5 : 2;
		ret.add(new ItemStack(TFCItems.wool, num));
		if (!familiarizedToday && this.familiarity <= FAMILIARITY_CAP)
		{
			familiarizedToday = true;
			this.playLivingSound();
	...
	public void setLocationAndAngles(double p_70012_1_, double p_70012_3_, double p_70012_5_, float p_70012_7_,
			float p_70012_8_)
	{
		if (posX == posY&& posY == posZ && posZ == 0)
		{

			float temp = TFC_Climate.getHeightAdjustedTemp(worldObj, (int) p_70012_1_, (int) p_70012_3_, (int) p_70012_5_);
			if (!this.isDomesticated && temp > 10)
			{
