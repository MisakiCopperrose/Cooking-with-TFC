...
public class EntityZombieTFC extends EntityZombie implements ICausesDamage, IInnateArmor
{
	...
	{
		if (par1 == 16)
			this.worldObj.playSoundEffect(this.posX + 0.5D, this.posY + 0.5D, this.posZ + 0.5D, "mob.zombie.remedy", 1.0F + this.rand.nextFloat(),
	...
	{
		if (ForgeHooks.onLivingAttack(this, ds, par2))
			return false;
	...
	{
		if ((this.worldObj.isDaytime()|| (!TFCOptions.enableDefaultCelestialAngle && TFC_Core.getCelestialDaylight(worldObj,(int)posX, (int)posY, (int)posZ,TFC_Time.getTotalTicks(),0) > 3)) && !this.worldObj.isRemote)
		{
	...
	private void summonAid(DamageSource ds)
	{
		...
		if (summonAid.getResult() == Result.DENY || this.rand.nextBoolean())
		{
			return;
		}
		else if (summonAid.getResult() == Result.ALLOW || entitylivingbase != null && this.worldObj.difficultySetting == EnumDifficulty.HARD
		&& this.rand.nextFloat() < this.getEntityAttribute(field_110186_bp).getAttributeValue())
		{
