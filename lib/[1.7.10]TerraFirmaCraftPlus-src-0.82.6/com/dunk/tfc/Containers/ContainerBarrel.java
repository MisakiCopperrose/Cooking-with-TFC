...
public class ContainerBarrel extends ContainerTFC
{
	...
	public void detectAndSendChanges()
	{
		...
		for (int var1 = 0; var1 < this.crafters.size() && guiTab == 0; ++var1)
		{
			...
			if(this.barrel.mode == 0 && this.barrel.storage[barrel.INPUT_SLOT] == null && !this.barrel.getWorldObj().isRemote)
			{
				//Check for other items we could transfer
				for(int i = 1; i < barrel.storage.length && this.barrel.storage[barrel.INPUT_SLOT] == null;i++)
				{
					if(barrel.storage[i] != null)
					{
						ItemStack temp = barrel.storage[i].copy();
						barrel.setInventorySlotContents(barrel.INPUT_SLOT, temp);
