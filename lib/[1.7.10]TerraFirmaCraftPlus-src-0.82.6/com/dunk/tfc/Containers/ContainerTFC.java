...
public class ContainerTFC extends Container
{
	...
	protected boolean mergeItemStack(ItemStack is, int slotStart, int slotFinish, boolean par4)
	{
		...
		boolean logPile = this instanceof ContainerLogPile;
		...
		if (is.isStackable())
		{
			while (is.stackSize > 0 && (!par4 && slotIndex < slotFinish || par4 && slotIndex >= slotStart))
			{
				...
				{
					int mergedStackSize = is.stackSize + getSmaller(slotstack.stackSize, slotLimit);
					...
					if (mergedStackSize <= is.getMaxStackSize() && mergedStackSize <= slotLimit)
					{
						is.stackSize = 0;
					...
					else if (slotstack.stackSize < is.getMaxStackSize() && slotstack.stackSize < slotLimit)
					{
						// Slot stack size is greater than or equal to the item's max stack size. Most containers are this case.
						if (slotLimit >= is.getMaxStackSize())
						{
						...
						else if (slotLimit < is.getMaxStackSize())
						{
		...
		if (is.stackSize > 0)
		{
			...
			while (!par4 && slotIndex < slotFinish || par4 && slotIndex >= slotStart)
			{
				slot = (Slot)this.inventorySlots.get(slotIndex);
				slotstack = slot.getStack();
				int slotLimit = logPile &&is != null && is.getItem() == TFCItems.thickLogs?1:slot.getSlotStackLimit();
				if (slotstack == null && slot.isItemValid(is) && slotLimit < is.stackSize)
				{
					ItemStack copy = is.copy();
					copy.stackSize = slotLimit;
					is.stackSize -= slotLimit;
					slot.putStack(copy);
					slot.onSlotChanged();
					merged = true;
