...
public class ItemProPick extends ItemTerra
{
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (!world.isRemote)
		{
			...
			if ((block == TFCBlocks.ore || block == TFCBlocks.ore1b ||	block == TFCBlocks.ore2 || block == TFCBlocks.ore3) && 
					world.getTileEntity(x, y, z) instanceof TEOre)
			{
				TEOre te = (TEOre) world.getTileEntity(x, y, z);
				if ((block == TFCBlocks.ore||block == TFCBlocks.ore1b) && rank == SkillRank.Master)
					meta = ((BlockOre) block).getOreGrade(te, meta);
				if (block == TFCBlocks.ore2) meta = meta + Global.oreSize;
				if (block == TFCBlocks.ore3) meta = meta + Global.oreSize + Global.ORE_MINERAL.length;
				tellResult(player, new ItemStack(TFCItems.oreChunk, 1, meta + ((BlockOre)block).damageOffset));
				return true;
			}
			...
			for (int i = -12; i < 12; i++)
			{
				for (int j = -12; j < 12; j++)
				{
					for(int k = -12; k < 12; k++)
					{
						...
						ItemStack ore = null;
						if (TFC_Core.isOre(block) && world.getTileEntity(blockX, blockY, blockZ) instanceof TEOre)
						{
							TEOre te = (TEOre) world.getTileEntity(blockX, blockY, blockZ);
							if (rank == SkillRank.Master)
								ore = new ItemStack(TFCItems.oreChunk, 1, ((BlockOre) block).getOreGrade(te, meta) + ((BlockOre)block).damageOffset);
							else
								ore = new ItemStack(TFCItems.oreChunk, 1, meta + ((BlockOre)block).damageOffset);
						}
						else if (block == TFCBlocks.ore2)
							ore = new ItemStack(TFCItems.oreChunk, 1, meta + Global.oreSize + ((BlockOre)block).damageOffset);
						else if (block == TFCBlocks.ore3)
							ore = new ItemStack(TFCItems.oreChunk, 1, meta + Global.oreSize + Global.ORE_MINERAL.length + ((BlockOre)block).damageOffset);
						else if (!TFC_Core.isGround(block)) // Exclude ground blocks to help with performance
						{
							Iterator iter = WorldGenOre.oreList.values().iterator();
							while (iter.hasNext())
