		...
		if (te.getWorldObj() != null)
		{
			...
			if (te.getIsWeaving() || te.getModel().stillWeaving)
			{
				if (TFC_Time.getTotalTicks() > te.getModel().tempTime)
				{
					...
					if (te.getModel().tempNum >= (te.getModel().lastClothIncrease + ((te.getModel().mod * (te.primitive?2:1)) / 2)))
					{
						te.getModel().clothIncrease = true;
						te.getModel().lastClothIncrease = (te.getModel().lastClothIncrease + ((te.getModel().mod* (te.primitive?2:1)) / 2)) % (te.getModel().mod* (te.primitive?2:1));
						
						te.finishWeaveCycle();
						te.getModel().stillWeaving = te.getIsWeaving();
					}
					te.getModel().tempNum = te.getModel().tempNum % (te.getModel().mod* (te.primitive?2:1));
				}
			}
			else{
				te.getModel().tempTime = TFC_Time.getTotalTicks();
			...
			if (te.getStackInSlot(1) != null) // Render completed cloth
				te.getModel().render(reqStringCount, reqStringCount, te.getModel().tempNum, false, te.getModel().mod * (te.primitive?2:1), te.getStringResource(), false, false, te);
			else // Render based on string, or empty loom
				te.getModel().render(stringCount, reqStringCount, te.getModel().tempNum, te.getModel().clothIncrease, te.getModel().mod * (te.primitive?2:1), te.getStringResource(), te.getIsWeaving(), te.getModel().stillWeaving, te);
