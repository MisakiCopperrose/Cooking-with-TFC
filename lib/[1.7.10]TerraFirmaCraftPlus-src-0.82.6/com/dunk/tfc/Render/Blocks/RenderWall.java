...
public class RenderWall  implements ISimpleBlockRenderingHandler
{
	...
	{
		boolean smoothMarble = wallBlock == TFCBlocks.wallSmoothMM && renderblocks.blockAccess.getBlockMetadata(x, y, z) == 5;
		boolean smoothMup =  renderblocks.blockAccess.getBlock(x, y+1, z) == TFCBlocks.stoneMMSmooth  && renderblocks.blockAccess.getBlockMetadata(x, y+1, z) == 5;
		boolean smoothMdown =  renderblocks.blockAccess.getBlock(x, y-1, z) == TFCBlocks.stoneMMSmooth  && renderblocks.blockAccess.getBlockMetadata(x, y-1, z) == 5;
		...
		{
			if(smoothMarble && smoothMdown)
			{
				renderblocks.setRenderBounds(0.15D, 0.0D, 0.15D, 0.85D, 0.2D, 0.85D);
				renderblocks.renderStandardBlock(wallBlock, x, y, z);
			}
			if(smoothMarble && smoothMup)
			{
				renderblocks.setRenderBounds(0.15D, 0.8D, 0.15D, 0.85D, 1D, 0.85D);
				renderblocks.renderStandardBlock(wallBlock, x, y, z);
			}
		}
