...
public class RenderFishTFC extends RenderLiving
{
	...
	{
		preRenderScale(par1EntityLiving, par2);
		Block blockBelow = par1EntityLiving.worldObj.getBlock((int) par1EntityLiving.posX,
				(int) par1EntityLiving.posY - 1, (int) par1EntityLiving.posZ);

		// float pitch = ((EntityFishTFC) par1EntityLiving).currentRenderPitch;
		// GL11.glRotatef(pitch, 1, 0, 0);
		if (par1EntityLiving instanceof EntityFishTFC)
		{
	...
	{
		if (par1EntityLivingBase instanceof EntityFishTFC)
		{
			...
			if (par1EntityLivingBase.deathTime > 0)
			{
				...
				// GL11.glRotatef(f3 *
				// this.getDeathMaxRotation(par1EntityLivingBase), 0.0F, 0.0F,
				// 1.0F);
			}
		}
	}

	protected void preRenderScale(EntityLivingBase par1EntityBass, float par2)
	{
