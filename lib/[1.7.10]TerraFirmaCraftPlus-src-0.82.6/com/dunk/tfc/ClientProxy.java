...
import com.dunk.tfc.Render.Blocks.RenderBeehive;
...
import com.dunk.tfc.Render.Blocks.RenderTeepee;
...
import com.dunk.tfc.Render.Blocks.RenderWattle;
import com.dunk.tfc.Render.Blocks.RenderWoodConstruct;
import com.dunk.tfc.Render.Blocks.RenderWoodSpear;
...
public class ClientProxy extends CommonProxy
{
	...
	public void registerRenderInformation()
	{
		...
		RenderingRegistry.registerBlockHandler(TFCBlocks.wattleRenderId = RenderingRegistry.getNextAvailableRenderId(), new RenderWattle());
		...
		RenderingRegistry.registerBlockHandler(TFCBlocks.beehiveRenderId = RenderingRegistry.getNextAvailableRenderId(), new RenderBeehive());
		...
		RenderingRegistry.registerBlockHandler(TFCBlocks.teepeeRenderId = RenderingRegistry.getNextAvailableRenderId(), new RenderTeepee());
		RenderingRegistry.registerBlockHandler(TFCBlocks.woodSpearRenderId = RenderingRegistry.getNextAvailableRenderId(), new RenderWoodSpear());
	...
	public int grassColorMultiplier(IBlockAccess par1IBlockAccess, int i, int j, int k)
	{
		...
		int var6 = 0;
		int var7 = 0;
		boolean tallGrass = par1IBlockAccess.getBlock(i,j,k) == TFCBlocks.tallGrass;
		for (int z = -1; z <= 1; ++z)
		{
			for (int x = -1; x <= 1; ++x)
			{
		...
		float bright = tallGrass?1.15f:1f;
		int ans = (Math.min((int)(bright*(var5 / 9)), 255)) << 16 | (Math.min((int)((bright)*(var6 / 9)), 255)) << 8 | Math.min((int)((bright)*(var7 / 9)), 255);
		return ans;
	}
