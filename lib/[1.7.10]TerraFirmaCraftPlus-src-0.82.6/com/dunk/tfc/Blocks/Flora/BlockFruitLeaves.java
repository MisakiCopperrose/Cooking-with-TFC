...
public class BlockFruitLeaves extends BlockTerraContainer
{
	...
	public boolean isFlowering(World world, int x, int y, int z)
	{
		if (!world.isRemote)
		{
			...
			if (te != null)
			{
				if (fi != null)
				{
					return fi.inBloom(TFC_Time.getSeasonAdjustedMonth(z));
				}
			}
		}
		return false;
	...
	public void Pollinate(World world, int x, int y, int z)
	{
		if (!world.isRemote)
		{
			if (isFlowering(world, x, y, z))
			{
				TEFruitLeaves te = (TEFruitLeaves) world.getTileEntity(x, y, z);
				int fruitType = te.fruitType;
				...
				float bloomLength = (fi.bloomFinish - fi.bloomStart) * TFC_Time.daysInMonth;

				te.pollination += 1f / bloomLength;
			}
		}
	}
	...
	public static boolean canStay(World world, int x, int y, int z)
	{
		...
		for (int i = 1; i >= -1; i--)
		{
			for (int j = 0; j >= -1; j--)
			{
				for (int k = 1; k >= -1; k--)
				{
					if (world.getBlock(i + x, j + y, k + z) == TFCBlocks.fruitTreeWood && world.getBlock(i + x, j + y + 1, k + z) != TFCBlocks.fruitTreeWood) // Only
																																								// branches
																																								// or
																																								// the
																																								// top
		...
		if (!world.isRemote)
		{
			...
			if (fi != null && (fi.inHarvest(TFC_Time.getSeasonAdjustedMonth(z)) || fi.inHarvest((TFC_Time.getSeasonAdjustedMonth(z) + 11) % 12) && (meta & 8) == 8))
			{
				...
				if (te != null && te.hasFruit)
				{
					...
					float output = 4 + (world.rand.nextFloat() * 12);
					if (te.pollination > 0)
					{
						output *= te.pollination + 1f;
					}
					te.pollination = 0;

					dropBlockAsItem(world, x, y, z, ItemFoodTFC.createTag(fi.getOutput(), Helper.roundNumber(output, 10)));
				}
			}
		}
	}
