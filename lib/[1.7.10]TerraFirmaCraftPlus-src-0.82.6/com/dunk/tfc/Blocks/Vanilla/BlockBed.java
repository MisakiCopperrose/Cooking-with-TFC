...
public class BlockBed extends BlockDirectional
{
		...
		else
		{
			...
			if (world.provider.canRespawnHere() && world.getBiomeGenForCoords(x, z) != TFCBiome.HELL)
			{
				...
				if (TFC_Core.isExposedToRain(world, x, y, z))
				{
					if (temp > 0)
					{
						TFC_Core.sendInfoMessage(player, new ChatComponentTranslation("tile.customBed.rain"));
					}
					else
					{
						TFC_Core.sendInfoMessage(player, new ChatComponentTranslation("tile.customBed.snow"));
					}
					return false;
				}
				else if (this == TFCBlocks.strawHideBed && ((temp < 0 && (bodyTemp.discomfort < 0 || bodyTemp.discomfort > 1)) || Math
						.abs(bodyTemp.discomfort) >= 2))
				{
					if (bodyTemp.discomfort < 0)
					{
						TFC_Core.sendInfoMessage(player, new ChatComponentTranslation("tile.customBed.tooCold"));
					}
					else
					{
						TFC_Core.sendInfoMessage(player, new ChatComponentTranslation("tile.customBed.tooWarm"));
					}
					return false;
				}
	...
	public IIcon getIcon(int par1, int par2)
	{
		...
		else
		{
			...
			return (i1 != 1 || l != 2) && (i1 != 0 || l != 3)
					? (l != 5 && l != 4 ? this.bedTopIcons[i1] : this.bedSideIcons[i1]) : this.bedEndIcons[i1];
		}
	}
	...
	/*
	 * Removed because it is trying to load an item texture that doesn't exist
	 * causing a slow down at launch
	 * 
	 * @Override public String getItemIconName(){ return "straw bed"; }
	 */
			...
			int par4)
	{
		...
		for (int k1 = 0; k1 <= 1; ++k1)
		{
			...
			for (int l2 = l1; l2 <= j2; ++l2)
			{
				...
				{
					if (World.doesBlockHaveSolidTopSurface(par0World, l2, par2 - 1,
							i3) && !par0World.getBlock(l2, par2, i3).getMaterial()
									.isOpaque() && !par0World.getBlock(l2, par2 + 1, i3).getMaterial().isOpaque())
					{
						if (par4 <= 0)
