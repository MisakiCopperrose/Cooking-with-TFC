...
public class BlockLiquidStatic extends BlockLiquid implements IFluidBlock
{
	...
	{
		if(this.flowing instanceof BlockCustomLiquid)
		{
			((BlockCustomLiquid)this.flowing).onNeighborBlockChange(world, x, y, z, b);
		}
	...
	public void updateTick(World world, int x, int y, int z, Random rand)
	{
		if(!world.isRemote)
		{
					...
					&& world.isAirBlock(x, y + 1, z))
			{
				...
				if(biome == TFCBiome.RIVER && rand.nextInt(100) < 25)
				{
					world.playSoundEffect(x, y, z, TFC_Sounds.RIVER, rand.nextFloat()*0.5f, rand.nextFloat() * 0.2f + 0.2f);
				}
				if(rand.nextInt(100) < 25 && world.getBlockLightValue(x, y, z) < 7 && TFC_Climate.getHeightAdjustedTemp(world, x, y + 1, z) > 2)
				{
					outerloop:
	...
	public Block getFlowing(World w, int x, int y, int z)
	{
		return this.flowing;
	}
	...
	{
		if(this.getMaterial() == Material.lava)
			icons = new IIcon[]{registerer.registerIcon(Reference.MOD_ID + ":" + "lava_still"), registerer.registerIcon("lava_flow"),registerer.registerIcon("lava_still")};
		else
			icons = new IIcon[]{registerer.registerIcon("water_still"), registerer.registerIcon("water_flow"),registerer.registerIcon("water_still")};
		this.getFluid().setIcons(icons[2], icons[1]);
	}
	...
	{
		return side != 0 && side != 1 ? this.icons[1] : side != 0? this.icons[0] : this.icons[2];
	}
