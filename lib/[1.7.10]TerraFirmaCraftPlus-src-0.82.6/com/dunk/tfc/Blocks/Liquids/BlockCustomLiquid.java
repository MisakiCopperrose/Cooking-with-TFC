...
public abstract class BlockCustomLiquid extends BlockDynamicLiquid implements IFluidBlock
{
	...
	public void onBlockAdded(World world, int x, int y, int z)
	{
		...
		this.checkForHarden(world, x, y, z, false);
	}
	 ...
	 */
	private void checkForHarden(World world, int x, int y, int z, boolean canHarden)
	{
		if (world.getBlock(x, y, z) == this || world.getBlock(x, y, z) instanceof BlockLiquidStatic && ((BlockLiquidStatic)world.getBlock(x, y, z)).getFlowing(world, x, y, z) == this)
			if (this.blockMaterial == Material.lava)
			{
				boolean flag = false;
	 ...
	 * Caves check if it's a colid surface Plains check if its grass or dirt
	 ...
	 *            Z position
	 * @param direction
	 *            The direction relative to the given position the plant wants
			...
			IPlantable plantable)
	{
		...
		case Beach:
			...
			boolean hasWater = world.getBlock(x - 1, y, z).getMaterial() == Material.water || world
					.getBlock(x + 1, y, z).getMaterial() == Material.water || world.getBlock(x, y, z - 1)
							.getMaterial() == Material.water || world.getBlock(x, y, z + 1)
	...
	public void setBlockforLava(World world, int x, int y, int z, int typeOfLava)
	{
		...
		Block lay = rockLayer3.block;

		if (lay == TFCBlocks.stoneIgIn && (meta == 2 || meta == 1))
			felsicLava = false;
		else if (lay == TFCBlocks.stoneIgEx && (meta == 1 || meta == 2))
			felsicLava = false;
	...
	public void registerBlockIcons(IIconRegister register)
	{
		if (this.blockMaterial == Material.lava)
		{
			this.getFluid().setIcons(register.registerIcon(Reference.MOD_ID + ":" + "lava_still"),
					register.registerIcon("lava_flow"));
		}
		else
		{
	...
	{
		Block block = world.getBlock(x, y, z);
		if (block == TFCBlocks.thatch || block == TFCBlocks.barrel || block == TFCBlocks.vessel || block == TFCBlocks.basket || block == TFCBlocks.berryBush || block == TFCBlocks.smokeRack || block instanceof BlockCustomDoor || block == TFCBlocks.ingotPile)
			return false;
		else
	...
	public void updateTick(World world, int x, int y, int z, Random rand)
	{
		...
		if (meta > 0)
		{
			...
			if (this.sourceBlockCount >= 2 && this.blockMaterial == Material.water && !TFCOptions.enableFiniteWater)
			{
				...
				if (world.getBlock(x, y - 1, z).getMaterial().isSolid() || world.getBlock(x, y - 1, z)
			...
			if (this.blockMaterial == Material.lava && meta < 8 && newHeight < 8 && newHeight > meta && rand
					.nextInt(4) != 0)
			{
				tickRate *= 4;
			}
		...
		if (this.canReplace(world, x, y - 1, z)) // Can flow directly down
		{
			if (this.blockMaterial == Material.lava && world.getBlock(x, y - 1, z).getMaterial() == Material.water)
			{
				...
				this.func_149799_m(world, x, y - 1, z); // Sizzling Sound &
														// Particles
				return;
			}
		...
		else if (meta >= 0 && (meta == 0 || this.canStay(world, x, y - 1, z))) // Can't
																				// flow
																				// down
		{
			newHeight = meta + b0;
		...
		if (this.blockMaterial == Material.lava && rand.nextInt(25) == 0 && TFC_Core.isExposed(world, x, y, z))
		{
			this.checkForHarden(world, x, y, z, true);
		}
	}
