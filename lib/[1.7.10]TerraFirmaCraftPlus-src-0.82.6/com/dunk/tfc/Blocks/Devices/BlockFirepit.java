...
public class BlockFirepit extends BlockTerraContainer implements IHeatSource
{
			...
			if (equippedItem != null)
			{
				...
				if (item instanceof ItemFirestarter || item instanceof ItemFlintAndSteel)
				{
					if ((TEFirepit) world.getTileEntity(x, y, z) != null)
					{
						...
						if(!this.drunkChanceInteract(entityplayer))
						{
							//burn the player
							if(te.fireTemp > 100)
							{
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random rand)
	{
		...
		if (meta >= 1)
		{
			...
			if (world.getBlock(x, y + 1, z) == TFCBlocks.vessel)
			{
				...
				if (rand.nextBoolean())
				{
					world.spawnParticle("smoke", -offset + f + f4 - 0.3F, f1, -offset + f2 + f5 + 0.3F, 0.0D, 0.0D, 0.0D);
					world.spawnParticle("flame", -offset + f + f4 - 0.3F, f1, offset + f2 + f5 + 0.3F, 0.0D, 0.0D, 0.0D);
					world.spawnParticle("smoke", offset + f + f5 + 0.3F, f1, -offset + f2 + f4 - 0.3F, 0.0D, 0.0D, 0.0D);
					world.spawnParticle("flame", offset + f + f5 + 0.3F, f1, offset + f2 + f4 - 0.3F, 0.0D, 0.0D, 0.0D);
				}
				else
				{
					world.spawnParticle("flame", -offset + f + f4 - 0.3F, f1, -offset + f2 + f5 + 0.3F, 0.0D, 0.0D, 0.0D);
				...
				{
					world.spawnParticle("flame", -offset + f + f5 + 0.3F, f1, -offset + f2 + f6 + 0.2F, 0.0D, 0.0D, 0.0D);
					world.spawnParticle("flame", -offset + f + f4 - 0.3F, f1, -offset + f2 + f6 + 0.1F, 0.0D, 0.0D, 0.0D);
				}
			}
			else
			{
	...
	/**
	 * Displays a flat icon image for an ItemStack containing the block, instead
	 * of a render. Using primarily for WAILA HUD.
	 */
	@Override
	...
	{
		// We'll be nice and do EntityLivingBase so it won't burn up dropped
		// items.
