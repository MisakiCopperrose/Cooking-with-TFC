...
public class BlockLoom extends BlockTerraContainer
{
	...
	public BlockLoom(boolean primitive)
	{
		super(Material.wood);
		this.setCreativeTab(TFCTabs.TFC_DEVICES);
		this.setBlockBounds(0.1f, 0, 0.1f, 0.9f, 1, 0.9f);
	...
	public void registerBlockIcons(IIconRegister iconRegisterer)
	{
		...
		this.primitiveIcon = new IIcon[4];
		this.primitiveIcon[0] = iconRegisterer.registerIcon(Reference.MOD_ID + ":" + "wood/stringTie");
		this.primitiveIcon[1] = iconRegisterer.registerIcon(Reference.MOD_ID + ":" + "wood/stringTie1");
		this.primitiveIcon[2] = iconRegisterer.registerIcon(Reference.MOD_ID + ":" + "wood/stringTie2");
	}

	public BlockLoom setPrimitive(boolean b)
	{
		this.primitive = b;
		return this;
	}
	...
	public IIcon getIcon(int side, int meta)
	{
		if(meta == 2000)
		{
			return primitiveIcon[side];
		}
		// When doing inventory item render, we increase the side by 10 so that
		// we can draw the hoops instead of planks
		if (side >= 10)
		{
			side -= 10;
			if (side == 0 || side == 1)
	...
	protected ItemStack createStackedBlock(int par1)
	{
		if(primitive)
		{
			return new ItemStack(TFCItems.primitiveLoom,1,0);
		}
		int j = 0;
	...
	public NBTTagCompound writeLoomToNBT(TELoom te)
	{
		...
		nbt.setBoolean("primitive", primitive);
		return nbt;
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		...
		{
			if(!this.drunkDenyInteract(player))
			{
				return false;
			}
