...
public class TFC_Time
{
	...
	public static float getPercentSummer(long days, int z)
	{
		...
		float ans = 0;
		if(day == summerDay + (float)(0.5f*daysInYear))
		{
			//It's winter
			ans = 0f;
		}
		...
		else if(day > summerDay && day <  summerDay + (float)(0.5f*daysInYear))
		{
			//
			ans = 1f -((day - summerDay)/(float)(0.5f*daysInYear));
		}
		else if(day < summerDay)
		{
			//This means we're counting up to summer
			//We add to the day the number of days after the winter solstice before the year turns over.
			ans = (day + ((float)(0.5f*daysInYear) - summerDay)) / (float)(0.5f*daysInYear);
		}
		else
		{
			//day > summerDay + (float)(0.5f*daysInYear)
			ans = (day - (summerDay + (float)(0.5f*daysInYear)))  / (float)(0.5f*daysInYear);
		}
		return z > 0? 1f - ans:ans;
		//return z <=0?1f - ((float)Math.abs(summerDay - day)/(float)(0.5f*daysInYear)):((float)Math.abs(summerDay - day)/(float)(0.5f*daysInYear));		
	}
	...
	public static boolean isSpring(int z)
	{
		...
		if(month == TFC_Time.MARCH || month == TFC_Time.APRIL || month == TFC_Time.MAY)
		{
			return true;
		}
		return false;
	...
	public static boolean isSummer(int z)
	{
		...
		if(month == TFC_Time.JUNE || month == TFC_Time.JULY || month == TFC_Time.AUGUST)
		{
			return true;
		}
		return false;
	...
	public static boolean isFall(int z)
	{
		...
		if(month == TFC_Time.SEPTEMBER || month == TFC_Time.OCTOBER || month == TFC_Time.NOVEMBER)
		{
			return true;
		}
		return false;
	...
	public static boolean isWinter(int z)
	{
		...
		if(month == TFC_Time.DECEMBER || month == TFC_Time.JANUARY || month == TFC_Time.FEBRUARY)
		{
			return true;
		}
		return false;
