...
public class WAILAData implements IWailaDataProvider
{
	...
	public ItemStack oreStack(IWailaDataAccessor accessor, IWailaConfigHandler config)
	{
		...
		if(accessor.getBlock() instanceof BlockOre)
		{
			meta += ((BlockOre)accessor.getBlock()).damageOffset;
		}
		if (TFC_Core.isOre(accessor.getBlock())) // Metals & Coal
		{
		...
		{
			itemstack = new ItemStack(TFCItems.oreChunk, 1, meta + Global.oreSize);
			if (meta == 5)
		...
		{
			itemstack = new ItemStack(TFCItems.oreChunk, 1, meta + Global.oreSize + Global.ORE_MINERAL.length);
			return itemstack;
		}
			...
			IWailaConfigHandler config)
	{
		...
		int unsealTime = accessor.getNBTInteger(tag, "UnsealTime");
		FluidStack fluid = FluidStack.loadFluidStackFromNBT(tag.getCompoundTag("fluidNBT"));
		BarrelRecipe recipe = BarrelManager.getInstance().findMatchingRecipe(inStack, fluid, sealed, te.getTechLevel(), te.isHeated(),te);
		...
		{
			currenttip
			.add(TFC_Core.translate("gui.Barrel.SealedOff") + " : " + TFC_Time.getDateStringFromHours(unsealTime));
		}
			...
			IWailaConfigHandler config)
	{
		...
		else if (accessor.getBlock() == TFCBlocks.ore2)
		{
			...
			{
			case 1:
				currenttip.add(TFC_Core.translate("gui.plaster"));
				break;
			case 2:
			case 3:
			case 6:
