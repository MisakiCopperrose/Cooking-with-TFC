...
public class TFCChunkProviderGenerate extends ChunkProviderGenerate
{
	...
	public Chunk provideChunk(int chunkX, int chunkZ)
	{
		...
		// surfaceRavineGen.generate(this, this.worldObj, chunkX, chunkZ,
		// idsBig, metaBig);// surface
		// ravineGen.generate(this, this.worldObj, chunkX, chunkZ, idsBig,
		// metaBig);// deep
	...
	public void populate(IChunkProvider chunkProvider, int chunkX, int chunkZ)
	{
		...
		for (x = 0; x < 16; x++)
		{
			for (z = 0; z < 16; z++)
			{
				...
				{
					if (worldObj.getBlock(x + xCoord, y, z + zCoord) == TFCBlocks.worldItem && ((TEWorldItem) (worldObj.getTileEntity(x + xCoord, y, z + zCoord))).snowLevel <= 7)
					{
	...
	// This will return a list containing the Enums which can spawn in this
	// chunk.
	...
	public static boolean isAnimalSpawnLegal(EnumAnimalSpawn animal, Random rand, TFCBiome biome, float temperature, float rain)
	{
		...
		if (animal.minRain <= rain && animal.maxRain >= rain && animal.minTemp <= temperature && animal.maxTemp >= temperature)
		{
			...
			float rainFactor = 1f - (Math.abs(rain - idealRain) / (idealRain - animal.minRain)); // When
																									// the
																									// rain
																									// is
																									// closer
																									...
																									// approaches
																									// 1
			float tempFactor = 1f - (Math.abs(temperature - idealTemp) / (idealTemp - animal.minTemp)); // When
																										// the
																										// temp
																										// is
																										// closer
	...
	public boolean canSnowAt(World world, int x, int y, int z)
	{
		...
		else
		{
			if (y >= 0 && y < 256 && world.getSavedLightValue(EnumSkyBlock.Block, x, y, z) < 10 && TFC_Time.getTotalMonths() > 1)
			{
				...
				if (((((var7.isAir(world, x, y, z) || var7.isReplaceable(world, x, y, z)) && TFCBlocks.snow.canPlaceBlockAt(world, x, y, z))
				|| world.getBlock(x, y, z) == TFCBlocks.worldItem) && !var6.isAir(world, x, y - 1, z) && var6.getMaterial().blocksMovement()))
					return true;
			}
			return false;
		}
	...
	public void generateTerrainHigh(int chunkX, int chunkZ, Block[] idsTop)
	{
		...
		this.biomesForGeneration = this.worldObj.getWorldChunkManager().getBiomesForGeneration(this.biomesForGeneration, chunkX * 4 - cRadius, chunkZ * 4 - cRadius,
		xSize + (2 * cRadius + 1), zSize + (2 * cRadius + 1));
	...
	private double[] initializeNoiseFieldHigh(double[] outArray, int xPos, int yPos, int zPos, int xSize, int ySize, int zSize)
	{
		...
		{
			this.parabolicField = new float[(2 * cRadius + 1) * (2 * cRadius + 1)];
			for (int counter1 = -cRadius; counter1 <= cRadius; ++counter1)
			{
				for (int counter2 = -cRadius; counter2 <= cRadius; ++counter2)
				{
					float parabolaHeight = 10.0F / MathHelper.sqrt_float(counter1 * counter1 + counter2 * counter2 + 0.2F);
					this.parabolicField[counter1 + cRadius + (counter2 + cRadius) * (2 * cRadius + 1)] = parabolaHeight;
		...
		for (int x = 0; x < xSize; ++x)
		{
			for (int z = 0; z < zSize; ++z)
			{
				...
				byte radius = (byte) cRadius;
				BiomeGenBase baseBiome = this.biomesForGeneration[x + radius + (z + radius) * (xSize + (2 * radius + 1))];

				for (int xR = -radius; xR <= radius; ++xR)
				{
					for (int zR = -radius; zR <= radius; ++zR)
					{
						BiomeGenBase blendBiome = this.biomesForGeneration[x + xR + radius + (z + zR + radius) * (xSize + (2 * radius + 1))];
						float blendedHeight = this.parabolicField[xR + radius + (zR + radius) * (2 * radius + 1)] / 2.0F;
						if (blendBiome.rootHeight > baseBiome.rootHeight)
							blendedHeight *= 0.5F;
						// Makes rivers less likely to be cut off
						if (blendBiome.rootHeight > baseBiome.rootHeight && baseBiome == TFCBiome.RIVER)
							blendedHeight *= 0.5F;
						...
						/*
						 * if(blendBiome == TFCBiome.RIVER) { blendedHeight =
						 * (blendedHeight + blendBiome.rootHeight)*0.5f; } else
						 * if(baseBiome == TFCBiome.RIVER) { blendedHeight =
						 * (blendedHeight + baseBiome.rootHeight)*0.5f; }
						 */
	...
	private void replaceBlocksForBiomeHigh(int chunkX, int chunkZ, Block[] idsTop, Random rand, Block[] idsBig, byte[] metaBig)
	{
		...
		for (int xCoord = 0; xCoord < 16; ++xCoord)
		{
			...
			for (int zCoord = 0; zCoord < 16; ++zCoord)
			{
				...
				float rain = rainCofs[zCoord] * (rainfallLayer[arrayIndexDL] == null ? DataLayer.RAIN_400.floatdata1 : rainfallLayer[arrayIndexDL].floatdata1);
				...
				if (rain > 125)
				{
					...
					variation[xCoord][zCoord] = rand.nextInt((int) (rain / 5) + 1) == 0;
		...
		for (int xCoord = 0; xCoord < 16; ++xCoord)
		{
			...
			for (int zCoord = 0; zCoord < 16; ++zCoord)
			{
				...
				surfaceBlock = rand.nextBoolean() ? TFC_Core.getTypeForSand(rock1.data1) : TFC_Core.getTypeForGravel(rock1.data1);
				...
				for (int height = 127; height >= 0; --height)
				{
					...
					if (idsBig[indexBig] == Blocks.stone)
					{
						...
						if (var13 == -1)
						{
							...
							for (int c = 1; c < 3; c++)
							{
								if (indexBig + c < idsBig.length && idsBig[indexBig + c] != surfaceBlock && idsBig[indexBig + c] != subSurfaceBlock
								&& idsBig[indexBig + c] != TFCBlocks.saltWaterStationary && idsBig[indexBig + c] != TFCBlocks.freshWaterStationary
							...
							int dirtH = Math.max(8 - (((height + 32)) / 16), 0) / ((Math.abs(zCoord + chunkZ * 16) > 27000) || height < (Global.SEALEVEL - 128 + 6) ? 1 : 2);

							// If the temperature will never get below 0 at this
							// altitude and latitude
							if (TFC_Climate.getMaxTemperature(worldObj, 0, height + 128, zCoord + chunkZ * 16) <= 0)
							{
								dirtH = 3;
								subSurfaceBlock =originalSubSurfaceBlock = Blocks.snow;
								alternateSubsurfaceMeta = 0;
								surfaceBlock  =originalSurfaceBlock= Blocks.snow;
								alternateSurfaceMeta = 0;
							}
							if (dirtH > 0)
							{
								...
								&& /*
									 * idsTop [ index + 1] != TFCBlocks .
									 * saltWaterStationary &&
									 */ dirtH > 0)
								{

									if (biome == TFCBiome.OCEAN || biome == TFCBiome.DEEP_OCEAN || biome == TFCBiome.BEACH || biome == TFCBiome.GRAVEL_BEACH)
									{
									...
									else if (height >= seaLevel - 1)
									{
										if (desert)
										{
											idsBig[indexBig] = localVariation ? originalSurfaceBlock : surfaceBlock;
											metaBig[indexBig] = localVariation ? originalSurfaceMeta : alternateSurfaceMeta; // (byte)TFC_Core.getSoilMeta(rock1.data1);
										}
										else
										{
											idsBig[indexBig] = !localVariation ? originalSurfaceBlock : surfaceBlock;
									...
									&& !cliffMap[arrayIndex]; c++)
									{
										...
										idsBig[newIndexBig] = desert && localVariation ? originalSubSurfaceBlock : subSurfaceBlock;
										metaBig[newIndexBig] = desert && localVariation ? originalSubSurfaceMeta : alternateSubsurfaceMeta;// (byte)TFC_Core.getSoilMeta(rock1.data1);

										if (c > 1 + ((dirtH - drainage.data1 / 2) / 2))
										{
											if (TFC_Climate.getMaxTemperature(worldObj, 0, offsetHeight + 128, zCoord + chunkZ * 16) < 0)
											{
						...
						|| height < seaLevel && idsTop[index + 1] == TFCBlocks.saltWaterStationary)
						{
																...
																// sand bottoms
							{
								if (idsBig[indexBig] != TFC_Core.getTypeForSand(rock1.data1) && rand.nextInt(5) != 0 )
								{
									...
									{
										idsBig[indexBig] = TFC_Core.getCobbleFromLayer(rock1.data1);
										metaBig[indexBig] = (byte) TFC_Core.getMetaFromLayer(rock1.data1);
									}
									else if (bioTemp > 25 && biome != TFCBiome.DEEP_OCEAN)
									{
										idsBig[indexBig] = TFC_Core.getTypeForSand(10); // Chalk
																							// sand
										metaBig[indexBig] = 10;
									}
									else if (bioTemp <= 25 && bioTemp > 10 && biome != TFCBiome.DEEP_OCEAN)
									{
										idsBig[indexBig] = TFC_Core.getTypeForSand(15); // Quartzite
																							// sand
										metaBig[indexBig] = 15;
									}
									else
					...
					else if (idsTop[index] == TFCBlocks.saltWaterStationary && biome != TFCBiome.OCEAN && biome != TFCBiome.DEEP_OCEAN && biome != TFCBiome.BEACH
		...
		for (int xCoord = 0; xCoord < 16; ++xCoord)
		{
			for (int zCoord = 0; zCoord < 16; ++zCoord)
			{
				...
				for (int height = 127; height >= 0; --height)
				{
					...
					{
						/*
						 * if(height > 64 && biome == TFCBiome.DEEP_OCEAN) {
						 * idsBig[indexBig] = TFCBlocks.saltWaterStationary;
						 * continue; }
						 */
