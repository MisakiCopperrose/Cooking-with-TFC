...
public class WorldGenCustomBigTree extends WorldGenTreeBase
{
	...
	{
		int radius = 2;
		boolean allowVines = (WorldGenForests.shouldPlantGrow(EnumForestPlant.VINE, rainfall, temperatureAvg, evt,
				random));
		...
		for (int i = -(radius + 1); i <= (radius + 1); i++)
		{
			for (int j = 0; j <= radius + 1; j++)
			{
				...
				{
					if (i * i + j * j + k * k <= radius * radius && world.getBlock(xCoord + i, yCoord + j, zCoord + k)
							.isReplaceable(world, xCoord + i, yCoord + j,
									zCoord + k) && !defLost && (!lost || random.nextInt(3) == 0))
					{
					...
					else if (j == 0 && world.getBlock(xCoord + i, yCoord + j, zCoord + k).isAir(world, xCoord + i,
							yCoord + j, zCoord + k) && !defLost && allowVines)
					{
						int distFromTrunk_X = tempSourceX - (xCoord + i);
						int distFromTrunk_Z = tempSourceZ - (zCoord + k);
						// Make sure we're not too high and not too close to the
						// trunk
						...
						{
							// world.setBlock(xCoord + i, yCoord + j + 0, zCoord
							// + k, Blocks.cobblestone, 0, 2);
							int meta = 0;
	...
	public boolean generate(World world, Random rand, int x, int y, int z)
	{
		...
		// gets out of hand
		if ((Math.abs(x) % 16 > 16 - distFromEdge || Math.abs(x) % 16 < distFromEdge || Math
				.abs(z) % 16 > 16 - distFromEdge || Math.abs(z) % 16 < distFromEdge) && !fromSapling)
		{
			return false;
		}
