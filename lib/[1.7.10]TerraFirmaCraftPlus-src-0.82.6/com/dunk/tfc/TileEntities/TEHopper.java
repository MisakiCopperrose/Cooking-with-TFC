...
public class TEHopper extends NetworkTileEntity implements IHopper
{
	...
	private SlotChest testSlot;

	public TEHopper()
	{
		testSlot = new SlotChest(null, 0,0, 0).addItemException(ContainerChestTFC.getExceptions());
	}
	...
	public void readFromNBT(NBTTagCompound nbt)
	{
		...
		this.stoneMeta = nbt.getInteger("stoneMeta");
		this.stoneType = nbt.getInteger("stoneType");
	...
	public void writeToNBT(NBTTagCompound nbt)
	{
		...
		nbt.setInteger("stoneType", stoneType);
		nbt.setInteger("stoneMeta", stoneMeta);
	...
	public void updateEntity()
	{
		...
		else if (this.worldObj != null && !this.worldObj.isRemote)
		{
			...
			{
				this.setCooldown(0);
				if (this.worldObj.getBlock(xCoord, yCoord, zCoord) != TFCBlocks.stoneHopper)
				{
					this.feed();
				}
			}
			...
			{
				if (pressBlock != null
				&& !(blockAbove instanceof BlockCobble || blockAbove instanceof BlockGravel || blockAbove instanceof BlockSand || blockAbove instanceof BlockDirt))
				{
	...
	private void press()
	{
		...
		if (item != null)
		{
			float pressWeight = getPressWeight(item);
			if (item.stackSize > 0)
				Food.setWeight(item, Food.getWeight(item) - pressWeight);// 0.64 per
																	// cycle
																	// leads to
																	// 250mB per
																	// stack of
			...
			{
				barrel.addLiquid(new FluidStack(getPressedFluid(item), 10));
			}
		}
	}
	...
	private Fluid getPressedFluid(ItemStack i)
	{
		if(i == null)
		{
			return null;
		...
		Item item = i.getItem();
		if(item == TFCItems.olive)
		{
			return TFCFluids.OLIVEOIL;
		}
		else if(item == TFCItems.grapes)
		{
			return TFCFluids.GRAPEJUICE;
		}
		else if(item == TFCItems.sugarcane)
		{
			return TFCFluids.CANEJUICE;
		}
		return null;
	}
	...
	public int hasPressableItem()
	{
		...
		for (int i = 0; i < storage.length; i++)
		{
			...
			else if (storage[i] != null && storage[i].getItem() == TFCItems.grapes)
			{
				amount += Math.floor(Food.getWeight(storage[i]));
			}
			else if (storage[i] != null && storage[i].getItem() == TFCItems.sugarcane)
			{
				amount += Math.floor(Food.getWeight(storage[i]));
			}
		}
	...
	public float getPressWeight(ItemStack i)
	{
		if(i == null)
		{
			return 0;
	...
	public boolean feed()
	{
		...
		{
			if (!this.isCoolingDown() && BlockHopper.checkMeta(this.getBlockMetadata()))
			{
	...
	private IInventory getOutputInventory()
	{
		...
		return searchForOutputInventory(this.getWorldObj(), this.xCoord + Facing.offsetsXForSide[i], this.yCoord + Facing.offsetsYForSide[i],
	...
	public static IInventory searchForOutputInventory(World world, double x, double y, double z)
	{
		...
		// If no block inventory is found then we look for entities that have an
		// inventory
		if (iinventory == null)
		{
			List list =
			world.getEntitiesWithinAABBExcludingEntity((Entity) null, AxisAlignedBB.getBoundingBox(x, y, z, x + 1.0D, y + 1.0D, z + 1.0D), IEntitySelector.selectInventories);
	...
	private static boolean canMergeStacks(ItemStack stack1, ItemStack stack2)
	{
		...
		: stack1.getItemDamage() != stack2.getItemDamage() ? false : stack1.stackSize > stack1.getMaxStackSize() ? false : ItemStack.areItemStackTagsEqual(stack1, stack2);
	}
	...
	public void handleDataPacket(NBTTagCompound nbt)
	{
		...
		if (nbt.hasKey("stoneType"))
		{
			this.stoneType = nbt.getInteger("stoneType");
		}
		if (nbt.hasKey("stoneMeta"))
		{
			this.stoneMeta = nbt.getInteger("stoneMeta");
		}
	}
